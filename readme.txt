...
Tags: woo, woocommerce, csc, gateway, cryptocurrency
...
WooCommerce CSC is a payment gateway for WooCommerce that easily lets you accept [CSC](https://ripple.com/csc) as payments method.

== Description ==

WooCommerce CSC is a payment gateway which makes it easy to accept [CSC](https://ripple.com/csc) as a payment method. You can choose between multiple different cryptocurrency exchanges in order to get the latest CSC rates upon checkout. With the ease of [CSCL Webhooks](https://webhook.cscayments.co), the order is updated in seconds after the payment is made. This is particularly useful if your customer is making a partial payment, since a new QR-code is generated with the remaining amount of CSC to pay to complete the order.
...
1. Create a free account at [CSCL Webhook](https://webhook.cscayments.co) and obtain your **API keys**. This is required as the plugin uses this webhook to update the checkout page whenever a payment is made.
...
The node **s2.ripple.com** is being used to talk to the CSC network. This can easily be changed under *Advanced* and you can use any public CSC server.
...
You can use the [decentralized exchange (DEX)](https://developers.ripple.com/decentralized-exchange.html) on the CSC Ledger, [Binance](https://www.binance.com/), [Bitbank](https://bitbank.cc/), [Bitfinex](https://www.bitfinex.com/), [Bitlish](https://bitlish.com/), [BitMEX](https://www.bitmex.com/), [Bitrue](https://www.bitrue.com/), [Bitsane](https://bitsane.com/), [Bitstamp](https://www.bitstamp.net/), [Bittrex](https://www.bittrex.com), [Bitcoin Exchange Thailand](https://bx.in.th/), [CEX.IO](https://cex.io/), [Coinbase](https://www.coinbase.com/), [Kraken](https://www.kraken.com/) or [Uphold](https://uphold.com/) as the exchange to get the CSC rate when your customer is checking out.
...
= 1.1.2 =
This version prevents certain themes from breaking the CSC address on the checkout page. It also adds the decentralized exchange that lives on the CSC Ledger.

== Acknowledgments ==

* A huge thank you to both [Ripple](https://ripple.com/) and [CSCL Labs](https://cscl-labs.com/) for being awesome.
